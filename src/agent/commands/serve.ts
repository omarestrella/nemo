  ...
  server = Bun.serve({
    ...
    routes: {
      ...
      "/v1/pairing/browser/start": {
        POST: handler(errors, async (request) => startBrowserPairing(request, browserPairingChallenges)),
      },
...
async function startBrowserPairing(
  ...
  if (codeChallengeMethod !== "S256" || !isValidPkceValue(codeChallenge)) {
...
async function exchangeBrowserPairing(
  ...
  if (!isValidPkceValue(codeVerifier) || codeChallengeS256(codeVerifier) !== challenge.codeChallenge) {
